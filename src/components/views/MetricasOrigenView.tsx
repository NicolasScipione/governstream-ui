...
const origenData = [
  ...
  { origen: 'Foro Productivo de la Zona Norte', cantidad: 234, porcentaje: 19, icon: Phone, color: '#22C55E' },
  { origen: 'Cámara de Gimnasios de Córdoba', cantidad: 112, porcentaje: 9, icon: Mail, color: '#EAB308' },
  { origen: 'Centro Comercial General Cabrera', cantidad: 66, porcentaje: 5, icon: MessageSquare, color: '#9CA3AF' },
];

const origenTendencia = [
  { mes: 'Ago', web: 78, presencial: 45, callcenter: 32, email: 15, municipalidad: 8 },
  { mes: 'Sep', web: 89, presencial: 52, callcenter: 38, email: 18, municipalidad: 10 },
  { mes: 'Oct', web: 95, presencial: 48, callcenter: 41, email: 22, municipalidad: 12 },
  { mes: 'Nov', web: 102, presencial: 55, callcenter: 45, email: 19, municipalidad: 11 },
  { mes: 'Dic', web: 88, presencial: 58, callcenter: 42, email: 20, municipalidad: 13 },
  { mes: 'Ene', web: 71, presencial: 54, callcenter: 36, email: 18, municipalidad: 12 },
];
      ...
      <div className="flex-1 overflow-auto p-6 space-y-6">
        ...
        <div className="grid grid-cols-5 gap-4">
          {origenData.map((origen) => {
            ...
            return (
              <div key={origen.origen} className="dashboard-card">
                <div className="flex items-center gap-3 mb-3">
                  <div 
                    className="w-10 h-10 rounded-lg flex items-center justify-center"
                    style={{ backgroundColor: `${origen.color}20` }}
                  >
                    <Icon className="w-5 h-5" style={{ color: origen.color }} />
                  </div>
        ...
        {/* Summary Table */}
        <div className="dashboard-card">
          <h3 className="text-base font-semibold mb-4">Resumen por Origen Institucional</h3>
          <table className="w-full text-sm">
            ...
            <tbody>
              {origenData.map((origen, index) => {
                const Icon = origen.icon;
                return (
                  <tr key={origen.origen} className={index % 2 === 0 ? 'bg-card' : 'bg-muted/30'}>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Icon className="w-4 h-4" style={{ color: origen.color }} />
